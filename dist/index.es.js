import { getCurrentScope, onScopeDispose, getCurrentInstance, onUnmounted, watch, unref, watchEffect, ref, defineComponent, onUpdated, withDirectives, openBlock, createElementBlock, normalizeStyle, createElementVNode, renderSlot, Fragment, renderList, vShow } from 'vue';
...
function fromResizeObserver(elRef, pluckTarget) {
  return fromEventPattern(pipe(unary$1, partial$1(useResizeObserver, [elRef]))).pipe(mergeAll(), pluck(pluckTarget));
}
function fromWindowScroll(elRef) {
  return fromEvent(window, "scroll", {
    passive: true,
    capture: true
  }, () => unrefElement(elRef)).pipe(filter$2((el) => Boolean(el)));
}
...
function accumulateAllItems(allItems, [{ pageNumber, items }, length]) {
  return pipe(concat$1(__, new Array(Math.max(length - allItems.length, 0)).fill(void 0)), remove$1(pageNumber * items.length, items.length), insertAll$1(pageNumber * items.length, items), slice$1(0, length))(allItems);
}
function getVisibleItems({ bufferedOffset, bufferedLength }, { columns, itemWidthWithGap, itemHeightWithGap }, allItems) {
  return pipe(slice$1(bufferedOffset, bufferedOffset + bufferedLength), addIndex$1(ramdaMap)((value, localIndex) => {
...
function accumulateBuffer(buffer, visibleItems) {
  ...
  return pipe(without$1(itemsToDelete), ramdaMap((item) => replaceMap.get(item) ?? item), concat$1(__, itemsToAppend))(buffer);
}
...
function pipeline({
  ...
  const resizeMeasurement$ = combineLatest([rootResize$, itemRect$], getResizeMeasurement).pipe(distinctUntilChanged(equals$1));
  const contentHeight$ = combineLatest([resizeMeasurement$, length$], getContentHeight);
  const windowScrollTo$ = scrollTo$.pipe(filter$2(complement$1(isNil$1)), switchMap((scrollTo) => combineLatest([of(scrollTo), resizeMeasurement$, rootResize$]).pipe(take(1))), map$1(([scrollTo, { columns, itemHeightWithGap }, rootEl]) => Math.floor(scrollTo / columns) * itemHeightWithGap + (rootEl.getBoundingClientRect().top + document.documentElement.scrollTop)));
  const bufferMeta$ = combineLatest([heightAboveWindow$, resizeMeasurement$], getBufferMeta()).pipe(distinctUntilChanged(equals$1));
  const itemsByPage$ = combineLatest([
    bufferMeta$,
    length$,
    pageSize$
  ]).pipe(mergeMap(apply$1(getObservableOfVisiblePageNumbers)), distinct(identity$1, merge(pageSize$, pageProvider$)), withLatestFrom(pageSize$, pageProvider$), mergeMap(apply$1(callPageProvider)), shareReplay(1));
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return withDirectives((openBlock(), createElementBlock("div", {
    ...
    (openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.buffer, (internalItem, index) => {
